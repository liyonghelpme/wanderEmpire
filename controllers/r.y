...
class RootController(BaseController):
    ...
    def fortunerank(self,type,off,num,uid):
        ...
        try:
            if type==0:
                rank1=DBSession.query(operationalData).order_by(operationalData.cae).order_by(operationalData.corn).all()
                rank1.sort(key=lambda x:(x.cae*1000+x.corn,x.lev))
    ...
    def warresult2(uid):
        ...
        for b in battleset3:#按战斗开始时间排序
            ...
            if b.uid==int(uid):#查询用户为攻击方
                ...
                lostcal=callost(poweru,powere,poweruu,poweree,1)
                if poweru>powere:#jingongfang shengli
                    ...
                    sss=str(b.uid)+',0,'+str(fpower)+','+str(powere)+','+str(poweru)+','+resourcegete#防御失败损失特殊物品及%5金币
                ...
                lostcal=callost(poweru,powere,poweruu,poweree,0)#防守
                if poweru>powere:
                    lostu=lostcal[0]
                    powerminus=lostu
                    expgotu=getexp(loste)#用户计算获得经验值
    ...
    def warrecord(self,uid):#对外接口，战绩
        ...
        return dict(wonlist=a1,lostlist=a2,warrecord=u.battleresult,won=uv.won,attack=uv.won+uv.lost,woninmap=uv.woninmap,lostinmap=uv.lostinmap,dewoninmap=uv.dewoninmap,delostinmap=uv.delostinmap)                                                        
    @expose('json')
